...
**Mock emitter subscriptions:** The SDK creates **two** `createSoliditySubscription` calls on `mockSignalEmitter`, with explicit `eventTopics` for **HealthSignal** and **MetricSignal** (`keccak256` of each signature). Subscribing with **four zero topics** and a normal contract emitter is rejected by the precompile (revert); do not rely on “omit topics = wildcard” for external emitters on testnet.
